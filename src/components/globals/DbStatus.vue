<template>
  <div>
    <el-popover placement="right-end" width="80%" trigger="hover">
      <div>
        <ul>
      ...
      <table>
        <thead>
          ...
          <th align="center">
            <el-button size="mini">Odeslat</el-button>
          </th>
        ...
        <tbody>
          <tr v-for="(w, i ) in who" :key="i">
            ...
            <td align="center">
              <el-checkbox v-model="message2" :value="message2">
                <v-icon color="blue darken-2">fa-bicycle</v-icon>
              </el-checkbox>
            </td>
            <td align="center">
              <el-checkbox v-model="message">
                <v-icon color="blue darken-2">message</v-icon>
              </el-checkbox>
            </td>
            <td align="center">
              <el-checkbox>
                <v-icon color="blue darken-2">email</v-icon>
              </el-checkbox>
            </td>
            <td align="center">
              <el-checkbox>
                <v-icon color="blue darken-2">sms</v-icon>
              </el-checkbox>
            </td>
          </tr>
        </tbody>
      ...
      <el-button
        slot="reference"
        class="ma-2 pa-2"
        v-bind:class="{ success: vse.pouzito<=1, warning: vse.pouzito > 1 }"
      >Db stav {{pouzito}}</el-button>
    </el-popover>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async db_who() {
      ...
      await DbStatusService.who({ idefix: this.$store.state.idefix }).then(
        res => {

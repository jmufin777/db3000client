<template>
  ...
  >
    ...
    >
      <el-row :gutter="0">
        <el-col :span="8" class="slot-a" style="font-size:10px">
          <slot name="a1" :sm="8" :md="8" :lg="8" :xl="8">
    ...
    <div class="elevation-20 drag01" style="height:97%;overflow-y:scroll; background:#FFF">
      <div v-if="demo">
        <h1>H1: {{ h1 }} H:{{ h }}</h1>
        <input type="number" v-model="h1" /> h:
        <input type="number" v-model="h" />
        <table border="5">
          <thead>
            <tr>
              <td>ID:</td>
              <td v-for="(w2,i2) in WinDows[0]" :key="i2" class="pa-2 ma-1">{{ i2 }}</td>
            </tr>
          </thead>
        ...
        <div>
          ...
          <br />txt:
          <input type="text" v-model="idx" style="background: red" />
        </div>
      </div>
      <slot></slot>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState(["isUserLoggedIn", "WinDows", "active", "zMax"]),
    idx: function() {
      return this.idxx();
    }
  },

  <template>
<el-row id="m381" style="overflow:scroll"  class="pa-2" >
  <el-col :span="24">
  <el-row  :gutter="100">
  ...
  <el-input prefix-icon="el-icon-search" id="search_381" autofocus clearable size="mini" v-model="search" placeholder="Prohledat tabulku">
  </el-input>
  </el-col>
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t381">
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t382">
  <form id="f381">
  ...
  >
      ...
      v-show="col.props.visible=='yes'"
      >
      <div :id="'d382_r_'+irow+'_c_'+icol"  class='dcell' >

        <input type="number" v-if="col.type =='number'"
        class=" px-4 cell " :id="'c382_r_'+irow+'_c_'+icol"
       ...
       >
        ...
        class="white px-4 cell seda" :id="'c382_r_'+irow+'_c_'+icol"
        v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
        :value="item[col.id]" style="width:100%;border:none;height:100%" readonly

        min="2007-06-01T08:30" max="3820-06-30T16:30"
        >
...
import List2Laminace from '@/services/List2StrojLaminaceService'
// import List2LaminaceVue from './List2Laminace.vue';
...
//    return
    if (this.isUserLoggedIn) {
      this.IsWaiting = true
      this.list = (await List2Laminace.all(this.user,'nic')).data
...
setTimeout(function(){
  ...
  destroyed () {
    if (document.getElementById("t382")){
        ///nejde
      //document.getElementById("t382").removeEventListener(document.getElementById("t302"),'keydown')
...
copyLine(nRow) {
   ...
   async saveLines(id){
     ...
     await List2Laminace.insert(this.user, {data: Posli, del: aDel })
     .then (res => {
   ...
   async newLine (nRow)  {
     var x
      ...
      this.Max = (await List2Laminace.all(this.user,'max')).data[0].kod*1 +10
...
//       this.$alert(ekeyCode)
   ...
   //return
    if (document.getElementById('search_381') && ekeyCode == 114 ){
       document.getElementById('search_381').focus()
     }
...
*/
   ...
   currid (itemId, colid) {
    ...
    async appendRow(index,rows) {
      ...
      try {
        await (List2Laminace.insert(this.user, this.form))

      } catch (err) {
        console.log(err)
      }

      await List2Laminace.all(this.user,'nic')
      .then(res => {

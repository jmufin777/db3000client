...
import List2Prace from '@/services/List2PraceService'
import f from '@/services/fce'
// import List2PraceVue from './List2MatSubSkup.vue';
...
//    return
    if (this.isUserLoggedIn) {
      this.IsWaiting = true
      this.list = (await List2Prace.all(this.user,'nic')).data
...
copyLine(nRow) {
   ...
   async saveLines(id){
     ...
     await List2Prace.insert(this.user, {data: Posli, del: aDel })
     .then (res => {
   ...
   async newLine (nRow)  {
     var x
      ...
      this.Max = (await List2Prace.all(this.user,'max')).data[0].kod*1 +10
...
//       this.aInfo.push(this.listEdits[eDelka][1])
   ...
   currid (itemId, colid) {
    ...
    async appendRow(index,rows) {
      ...
      try {
        await (List2Prace.insert(this.user, this.form))

      } catch (err) {
        console.log(err)
      }

      await List2Prace.all(this.user,'nic')
      .then(res => {

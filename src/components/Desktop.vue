<template >
  <v-app id="inspire">
    ...
    >
      ...
      <menu-nav
        :xmenu="xMenuFinal"
        :xgroup="people1001"
        @mouseenter.native="prijdi()"
        @mouseleave.native="opust()"
  ...
  <el-dropdown-menu v-if="this.compa.length>0" slot="dropdown" >
     <el-dropdown-item v-for="(compaE, iE) in compa " :key="iE" :command="compaE[0].modul">{{ compaE[0].title }}</el-dropdown-item>
      ...
      <div
        v-if="this.compa.length>0"
        style="background:f5f5f5;max-width:50%;border:none;"
        class="ml-4 px-2 elevation-0"
      >
        <el-tabs
          v-model="radio3t"
          @tab-click="radio3_atab"
          closable
          @tab-remove="removeCompa"
          style="background:f5f5f5"
          lazy
        >
          <el-tab-pane
            v-for="(compaE, iE) in compa "
            :key="iE"
            :name="compaE[0].modul"
            :label="compaE[0].title"
          ></el-tab-pane>
      ...
      <el-dropdown v-if="this.openWins.length>0" @command="winFocus">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item
            v-for="(openWin, iO) in openWins"
            :key="iO"
            :command="openWin"
          >{{openWin.name}}</el-dropdown-item>
...
Moduly:
      ...
      <el-tooltip placement="bottom" effect="light">
        <div slot="content">
          ...
          <span
            class="ma-2 pa-2 blue"
          >level: {{ level }} id: {{idefix}} login: {{ $store.state.user }}</span>
        </div>
    ...
    <v-content v-if="cont" style="padding-top: 5px">
      <v-container grid-list-md>
        <v-layout v-if="lay" row wrap>
          <div
            style="position:absolute;top:0px;left:0px;height:100%;width:99%;background-color:#f5f5f5;opacity:1;z-index:1"
          >
 ...
 @dragging
            ...
            >
              ...
              <el-button
                ...
                size="mini"
                class="elevation-20"
                @click="closeModule(comp[0].modul)"
              ></el-button>
              <keep-alive>
            ...
            <el-row v-if=" radio2==10" :gutter="0">
              <draggable
                v-model="compa"
                :options="{group:{ name:'app_move' }}"
                @start="drag=true"
                @end="drag=false"
                :move="chooseItem"
              >
                <el-col
                  v-for="(comp_app, iapp) in compa"
                  :key="iapp"
                  :span="myFlex || 24/ compa.length"
                >
                  <el-card shadow="hover">
                    <el-tooltip placement="bottom" effect="light">
                      <div slot="content">Vytvori nezavislou kopii v samostatne okne</div>
                      <el-button
                        v-if="radio3>'' && radio2==10"
                        type="error"
                        icon="el-icon-upload2"
                        size="mini"
                        class="elevation-20"
...
export default {
  ...
  watch: {
    ...
    $route(desktop) {
      ...
      this.compa.unshift([
        { modul: atmp[idx], title: atmp[idx - 3], where: atmp[idx + 2] }
      ]);
  ...
  methods: {
    ...
    radio3_atab: function() {
      //  alert(this.radio3t)
      if (
        this.radio3t === "CloseAllWin" &&
        confirm("Ukoncit vsechny aplikace ?")
      ) {
        this.compa = [];
      }
    ...
    menu_switch(par) {
      if (par.length >= 6) {
        if (par[5] === "window") {
          ...
          this.openWins.push(
            window.open(route.href, par[3], "width=400,height=400")
          );
          return;
        }
      }
  ...
  mounted() {
    ...
    if (this.$store.state.user == "host") {
      setTimeout(function() {
        self.drawer = false;
        self.compa.unshift([
          { modul: "test-pet", title: "F", where: "par[5]" }
        ]);
      }, 500);
    } else {

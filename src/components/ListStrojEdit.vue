...
<!-- Step: {{step2}} / {{ step }}  /{{ idefixThis }} -->
      ...
      <!-- <form  id="form8210" ref="form3" :model="form3" label-width="70px" :label-position="labelPosition"   class="demo-ruleForm is-success" > -->
...
<!-- <td :style="pof(Sirka*0.75,12)"  class="pl-2 leva">{{itemFull.nazev}}</td> -->
        ...
        <table slot="headl" :style="pof(Sirka*0.2,100)">
            <thead  >
              <tr>
              <th :style="pof(Sirka*0.2,100)" colspan="2">Nazev</th>

              </tr>
            </thead>
        </table>

        <table slot="bodyl" :style="pof(Sirka*0.23,100)" class="ml-0 mr-0 white">
        <tbody  >
...
<!-- Radek s inkousty !-->
    ...
    <el-row >
      <el-col :span="24">
        <v-btn v-if="addMod==true" @click="insertMod" small :id="'bA'+ID"> Vlozit </v-btn>
        <v-btn v-if="editMod==true" @click="saveMod(strojmod)"  small :id="'bA'+ID"> Zmena </v-btn>
    ...
    async resetForm(formName) {
        ...
        if (self.isCopy == true) {
          ...
          //alert(JSON.stringify(self.list_backup.data.strojmod))
           ...
           if (self.IsZmena && confirm('Ulozit data ?')) {

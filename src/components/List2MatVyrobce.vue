    ...
    <el-col :span="24">
      ...
      <div style="height:100%;overflow:scroll" class="mt-0" :id="'t' + objId1">
        <el-row style="backgroud: white">
          <el-col :span="2" class="mth stred">Akce</el-col>

          <el-col :span="22">
            <el-col
              v-for="( col, i0 ) in cols"
              :key="col.id"
              :span="col.span"
              class="mth stred"
            ...
            >
              {{col.title}}
              <i
                v-if="col.sort && col.sort=='asc'"
                @click="sortByKey(col.id,'desc')"
                class="el-icon-upload2"
              ></i>
              <i
                v-if="!col.sort || col.sort=='desc'"
                @click="sortByKey(col.id,'asc')"
            ...
            <el-col :span="2" class="mth stred">X</el-col>
          </el-col>
        </el-row>
              ...
              <el-col :span="2">
                ...
                >
                  <button
                    ...
                    @click="copyLine(irow)"
                  >
                    <i class="el-icon-document" size="mini"></i>
                  </button>
                  <button
                    type="button"
                    style="width:30%;height:8px"
              ...
              <el-col :span="22">
                ...
                >
                  <div :id="'d'+ objId2+ '_r_'+irow+'_c_'+icol" class="dcell">
                    ...
                    />
                    <select
                      v-else-if="col.type=='selectone'"
                      v-model="list[irow][col.id]"
                      class="px-0 cell"
                      :id="'c' + objId2+ '_r_'+irow+'_c_'+icol"
                      v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
                    ...
                    <input
                      type="text"
                      v-else
                      class="px-0 cell pl-1"
                      :id="'c' +objId2 +'_r_'+irow+'_c_'+icol"
                      v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
                      :value="item[col.id]"
                      style="width:100%;border:none;height:100%;text-align:left"
                      readonly
                    />

                    <input
                      type="date"
                      v-if="col.type =='datetime-local' && false"
                      class="white px-0 cell seda"
                      :id="'c'+ objId2 +'_r_'+irow+'_c_'+icol"
                      v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
                      :value="item[col.id]"
                      style="width:100%;border:none;height:100%"
                      readonly
                      min="2007-06-01T08:30"
                      max="8120-06-30T16:30"
                    />
                  </div>
                </el-col>
                <el-col :span="2">
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(function() {
      ...
      if (!document.getElementById(new_id).type == "number") {
        document.getElementById(new_id).selectionEnd = document.getElementById(
          new_id
        ).selectionStart;
      }
  ...
  methods: {
          ...
          setTimeout(function() {
            ...
            if (!document.getElementById(new_id).type == "number") {
              document.getElementById(
                new_id
              ).selectionEnd = document.getElementById(new_id).selectionStart;
            }
            //setTimeout(function(){
            f.changeClass(newObal, "dcell", "dcell_edit");
            document.getElementById(new_id).focus();
    ...
    async newLine(nRow) {
      ...
      this.Max =
        (await List2MatVyrobce.all(this.user, "max")).data[0].kod * 1 + 10;
    ...
    deleteLine(nRow) {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          if (eof == true) {
            new_id =
              "c" + self.objId2 + "_r_" + (self.list.length - 1) + "_c_" + 1;
          }
          if (top == true) {
            ...
            setTimeout(function() {
              if (!document.getElementById(new_id)) {
                ...
                if (!document.getElementById(new_id)) {
                  ...
                  setTimeout(function() {
                    self.isWrite = false;
                    var newObal = document.getElementById(
                      "d" + new_id.substring(1)
                    );
                    document.getElementById(new_id).focus();
                    document
                    ...
                    if (!document.getElementById(new_id).type == "number") {
                      document.getElementById(
                        new_id
                      ).selectionEnd = document.getElementById(
                        new_id
                      ).selectionStart;
                    }
                    f.changeClass(newObal, "dcell", "dcell_edit");
    ...
    async appendRow(index, rows) {
      ...
      await List2MatVyrobce.all(this.user, "nic").then(res => {
        //this.info= res
        this.list = res.data;
    ...
    setCurrent(row) {
      //Zavolat s cislem radky,treba tlacitkem,ted j nepotrenuju
  ...
  computed: {
    ...mapState(["isUserLoggedIn", "user", "xMenuMain", "level", "idefix"])
  }
};
</script>
<style scoped>

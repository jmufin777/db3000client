<template>
  ...
  <div
    style="position:relative;font-size:100%; border:1px solid black;border-right:none;min-height:18em"
    class="pt-1 pl-0 ml-1 pb-0"
  ...
  >
    ...
    <span v-if="true && kalkulaceid==KalkulaceThis">
      ...
      <div
        v-cloak
        style="max-height:10em;overflow-y:scroll;position:absolute;z-index:1000"
        v-if="form.showtxt && (getType()=='Mat1'  || getType()=='Laminace' || getType()=='Kasir' || getType()=='Jine'  )
        ...
        >
          ...
          <tr class="mt-1 green" v-for="(item, i) in filtrDat" :key="i">
            <td>
              <a
                :href="'#'"
                :id="'seek1_'+ID+'_list_'+i"
                @keydown="seznam('seek1_'+ID+'_list_'+i,1,$event)"
                @click="form.txt= item['nazev']; form.showtxt=false; setCol(item)"
                @focus="lastFocus='seek1_'+ID"
              >
                <v-card class="silver pa-0 ma-0" style="width:99%;">
                  <table style="width:100%" class="pa-0 ma-0">
                    <tr>
                      <td style="width:80%;height:100%" class="pa-0 ma-0 grey lighten-5">
                        <v-card-text
                      ...
                      <td style="width:20%;height:100" class="pa-0 ma-0 grey lighten-5">
                        <v-card-text
                          style="font-size:90%;text-align:center;width:100%;height:100%"
                          @click="form.txt= item['nazev']; form.showtxt=false; setCol(item)"
      ...
      <div
        style="min-height:5px;max-height:10em;overflow-y:scroll;position:absolute;z-index:51001"
        v-if="form.showtxtStroj && (getType()=='Laminace' || getType()=='Kasir' ||  getType()=='Rezani' || getType()=='Baleni' || getType()=='Jine') && filtrDataStro.length > 0"
      ...
      <div
        style="min-height:5px;max-height:10em;overflow-y:scroll;position:absolute;z-index:51001"
        v-if="form.showtxtPrace && (getType()=='Externi' ) && filtrDataPrac.length > 0"
        class="elevation-12 teal"
        ...
        <table width="100%" v-if="form.showtxtPrace && (getType()=='Externi'  ) ">
          <tr class="mt-0 pa-0 grey lighten-4" v-for="(item1, i1) in filtrDataPrac" :key="i1">
            <td class="pa-0 ma-0 pl-1 grey lighten-4">
              <a
                :href="'#'"
                :id="'seek4_'+ID+'_list_'+i1"
                @keydown="seznam('seek4_'+ID+'_list_'+i1,1,$event)"
                @click="form.txtPrace= item1['prace']; form.showtxtPrace=false; setColPrace(item1)"
                @focus="lastFocus='seek4_'+ID"
              >
                <v-card class="grey lighten-4 pa-0 ma-0" style="width:99%;">
                  <table style="width:100%" class="pa-0 ma-0">
                    <tr>
      ...
      <div
        style="min-height:5px;max-height:10em;overflow-y:scroll;position:absolute;z-index:51001"
        v-if="form.showtxtDod && (getType()=='Externi' ) && filtrDataDo.length > 0"
        class="elevation-12 teal"
        ...
        <table width="100%" v-if="form.showtxtDod && (getType()=='Externi'  ) ">
          <tr class="mt-0 pa-0 grey lighten-4" v-for="(item1, i1) in filtrDataDo" :key="i1">
            <td class="pa-0 ma-0 pl-1 grey lighten-4">
              <a
                :href="'#'"
                :id="'seek5_'+ID+'_list_'+i1"
                @keydown="seznam('seek5_'+ID+'_list_'+i1,1,$event)"
                @click="form.txtDod= item1['firma']; form.showtxtDod=false; setColDod(item1)"
                @focus="lastFocus='seek5_'+ID"
              >
                <v-card class="grey lighten-4 pa-0 ma-0" style="width:99%;">
                  <table style="width:100%" class="pa-0 ma-0">
                    <tr>
      ...
      <!--    // && ( filtrDataStroj().length>1|| (filtrDataStroj().length==1 && filtrDataStroj()[0].nazev!=form.txtStroj) //-->
      <div
        style="min-height:5px;max-height:10em;overflow-y:scroll;position:absolute;z-index:510015;border:1px solid silver"
        v-if="form.showtxtStroj1 && ( getType()=='Rezani' ) && filtrDataStro1.length > 0 "
        class="elevation-2 grey lighten-4 pa-0 ma-0"
      ...
      >
        ...
        <table
          width="100%"
          v-if="form.showtxtStroj1 && (  getType()=='Rezani' || getType()=='Baleni') "
        >
          <tr class="mt-0 pa-0" v-for="(item2, i2) in filtrDataStro1" :key="i2">
            <td class="ma-0 pa-0">
              <a
                :href="'#'"
                :id="'seek3_'+ID+'_list_'+i2"
                @keydown="seznam('seek3_'+ID+'_list_'+i2,$event)"
                @click="form.txtStroj1= item2['nazev_stroj']; form.showtxtStroj1=false; setColStroj1(item2)"
                @focus="lastFocus='seek3_'+ID"
              >
                <v-card class="grey lighten-4 pa-0 ma-0" style="width:99%;">
                  <table style="width:100%">
                    <tr>
                      <td style="width:100%" class="ma-0 pa-0 grey lighten-4">
                        <v-card-text
                          style="font-size:90%;text-align:left;width:100%"
                          @click="form.txtStroj1= item2['nazev_stroj']; form.showtxtStroj1=false; setColStroj1(item1)"
                          :class="{'blue lighten-5':1==1, 'orange lighten-5':1>1}"
                          class="grey lighten-4 pa-0 ma-0 pl-1"
                        >{{ item2['nazev_stroj'] }}</v-card-text>
                      </td>
                    </tr>
                  </table>
                </v-card>
    ...
    <form>
      ...
      <v-card
        style="width:100%;height:35px;float:none;border-radius:25px;border-top-right-radius:10px"
        class="pa-0 ma-0"
        :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
      >
        <v-card-text
          style="width:100%;z-index:900000; text-align:left "
          class="pa-0 pt-0"
          :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
        >
          <div
            style="width:100%;float:none;border-radius:25px;border-top-right-radius:30px !important;border: solid 0px black !important"
          >
            <table
              class="pa-0 ma-0"
              :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
              style="width:100%;border:solid 0px black"
            >
              <tr class="pa-0 ma-0 grey lighten-3">
                <td
                  width="80%"
                  class="pa-0 ma-0 pt-0 pb-0"
                  :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
                >
                  <el-dropdown
                    v-if="!(ITEM1.status==1)"
                    ...
                    trigger="click"
                    @command="zmenaType"
                    style="width:100;height:80% !important"
                    :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
                  >
                    {{getType()}}
                    <el-dropdown-menu
                ...
                <td
                  ...
                  style="border-top-right-radius:30px;text-align:right"
                  :class="{'blue lighten-4': $store.state.KalkulaceColThis==getId(),'grey lighten-3': $store.state.KalkulaceColThis!==getId()}"
                >
                  <button
                    type="button"
                    :disabled="ITEM1.status==1"
                    style="width:20%;height:26px;text-align:right"
                    class="px-0 cell"
                    @click="deleteCol()"
                  >
      ...
      <!-- jen rezani //-->
      <v-card
        style="width:100%;max-height:60px;float:none"
      ...
      <v-card
        style="width:100%;float:none;max-height:60px;border:none"
        class="pa-0 pt-0 elevation-0"
        v-if="getType()!='Externi' && getType()!='DTP' && !getType().match(/Rez/) && !getType().match(/Baleni/)"
      >
      ...
      >
        <v-card-text style="width:80%; text-align: left" class="pa-0 pl-1">
          <select
            v-if="getType()!=='Mat1'"
            v-model="form.tisk"
            @change="saveVuexData(); classJarda('sel1_'+ID)"
            :id="'sel1_'+ID"
            @keydown="classJarda('sel1opt_'+ID+'_'+form.tisk)"
            style="color:black;font-color:black;border: 1px solid white !important;font-size:110%"
            class="white lighten-2 pl-0 pr-2 pt-0 pb-0"
            :disabled="ITEM1.status==1"
          >
            <option
              v-for="(a,b ) in Tisk"
      ...
      <v-card
        style="width:100%;float:none;height:8em;overflow:scroll;border:none"
        class="pa-0 elevation-0"
        v-if="getType()=='Externi'"
      >
        <!-- Puvodni externi //-->
        <v-card-text style="width:100%;" class="pa-0">
          <!-- :style="'height:'+(17*f1.entrcount(form.txtStroj))+'px'" -->
        ...
        <v-card-text style="width:100%;text-align:left;" class="pa-0 pl-0">
          <table class="pa-0">
            <tr>
              <td class="pl-1 pt-0 pb-0" style="width:20%">Naklad/ks</td>
              <td style="width:30%" class="prava">
                <input
                  type="number"
                  :disabled="ITEM1.status==1"
                  v-model="form.ext_naklad_ks"
                  size="mini"
                  style="width:90%; height:15px; text-align:right;border-bottom: dotted 1px black;font-size:110%"
                  class="tdl tdn elevation-0 pr-1"
                  @click="readVuexData"
                  @change="form.ext_celkem=form.ext_naklad_ks*form.ext_pocet_ks ;saveVuexData()"
                />
              </td>
              <td class="pl-1" style="width:20%">Pocet</td>
              <td class="prava" style="width:30%">
                <input
                  type="number"
                  :disabled="ITEM1.status==1"
                  v-model="form.ext_pocet_ks"
                  size="mini"
                  style="width:90%; height:15px; text-align:right;;font-size:110%"
                  class="tdl tdn elevation-0 pr-1"
                  @click="readVuexData"
                  @change="form.ext_celkem=form.ext_naklad_ks*form.ext_pocet_ks;saveVuexData()"
                />
            ...
            <tr>
              <td class="pl-1" style="width:20%">Celkem</td>
              <td class="prava" style="width:30%">
                <input
                  type="number"
                  :disabled="ITEM1.status==1"
                  v-model="form.ext_celkem"
                  size="mini"
                  style="width:90%; height:15px; text-align:right;font-size:110%"
                  class="tdl tdn elevation-0 pr-1"
                  @click="readVuexData"
      ...
      <v-card
        style="width:100%;float:none;height:7em;overflow:scroll"
        class="pa-0"
        v-if="getType()=='DTP'"
      >
            ...
            <tr>
              ...
              <td style="width:40%">
                <input
                  ...
                  size="mini"
                  placeholder="Popis"
                  style="width:100%; height:15px; text-align:left"
                  class="tdl tdn elevation-0 pl-1"
                />
                <input
                  type="number"
                  :disabled="ITEM1.status==1"
                  v-model="form.prodejDTP"
                  size="mini"
                  placeholder="Prodej DTP"
                  style="width:100%; height:15px; text-align:right;font-size:110%"
                  class="tdl tdn elevation-0 pl-1"
              ...
              <td style="width:50%" colspan="1" class="pl-2">
                <table>
                  <tr>
                    <td>
                      <select
                        v-if="true || getType().match(/Baleni/)"
                        v-model="form.baleni"
                        @change="saveVuexData(); classJarda('sel2_'+ID)"
                        :id="'sel2_'+ID"
                        @keydown="classJarda('sel2opt_'+ID+'_'+form.Baleni)"
                        style="width:100%;color:black;font-color:black;border: 1px solid white !important;font-size:110%"
                        class="white lighten-2 pl-0 pr-2 pt-0 pb-0"
                        :disabled="ITEM1.status==1"
                      >
                        <option
      ...
      <v-card
        style="width:100%;float:none;position:absolute;bottom:0px;height:2em;vertical-align: text-bottom;"
        class="white"
        v-if="!getType().match(/Balenixx/)"
      >
        <v-card-text style="text-align:left;width:100%;vertical-align: text-bottom;" class="pa-0">
          <table>
            <tr>
              <td style="width:20%">&nbsp;Naklad&nbsp;&nbsp;</td>
              <td style="width:30%">
                <input
                  type="number"
                  :disabled="ITEM1.status==1"
      ...
      <v-card style="width:20%;float:left;display:none">
        <v-card-text style="text-align:left;width:100%;" class="pa-0"></v-card-text>
      </v-card>
...
export default {
  ...
  created() {
    ...
    eventBus.$on("Rend", server => {
      if (
        self.form.showtxt ||
        self.form.showtxtStroj ||
        self.form.showtxtStroj1
      ) {
        if (self.timeout) {
    ...
    async nactiDb(ihned = false) {
      ...
      setTimeout(function() {
        ...
        if (
          ...
          if (
            self.Kalkulace[self.k_id()].data.FormatSirka == 0 ||
            self.Kalkulace[self.k_id()].data.FormatVyska == 0
          ) {
            return;
          }
          self.countZmen++;
          ...
          var q1 = SQL.getMatList(
            self.Kalkulace[self.k_id()].data.Menu1Value,
            self.Kalkulace[self.k_id()].data.FormatSirka,
            self.Kalkulace[self.k_id()].data.FormatVyska
          );
...
*/
    ...
    async fokus(kdezejsem) {
      ...
      if (kdezejsem.match(/prace/)) {
        if (
          !self.Col.hasOwnProperty("DataPrace") ||
          self.Col.DataPrace.length == 0
        ) {
          var q2 = SQL.getPrace();
          self.Col.DataPrace = await self.qPrace(q2);
        }
        await self.filtrDataPrace();
      } else if (kdezejsem.match(/dod/)) {
        if (
          !self.Col.hasOwnProperty("DataDod") ||
          self.Col.DataDod.length == 0
        ) {
          var q2 = SQL.getDod();
      ...
      } else if (kdezejsem.match(/mat/)) {
        if (
          !self.Col.hasOwnProperty("data") ||
          self.Col.data.length == undefined ||
          self.Col.data.length == 0
        ) {
      ...
      if (kdezejsem.match(/stroj$/)) {
        ...
        if (self.form.showtxtPrace) self.form.showtxtPrace = false;
        if (self.form.showtxtDod) self.form.showtxtDod = false;
      } else if (kdezejsem.match(/stroj1$/)) {
        if (self.form.showtxt) self.form.showtxt = false;
        if (self.form.showtxtStroj) self.form.showtxtStroj = false;
        //if (self.form.showtxtStroj1 ) self.form.showtxtStroj1=false
        if (self.form.showtxtPrace) self.form.showtxtPrace = false;
    ...
    saveVuexData(aa) {
      ...
      self.$store.dispatch("replaceKalkCol", {
        kalkulaceid: self.k_id(),
        idxCol: self.getIndex(),
        data: {
    ...
    readVuexData() {
      ...
      var neco =
        self.$store.state.Kalkulace[self.k_id()].sloupecid[self.getIndex()];
    ...
    async filtrDataOld() {
      ...
      try {
        neco = self.Col.data.filter(
          el =>
            //(el.nazev).toUpperCase().match(self.form.txt.toUpperCase())  || self.form.txt ==''
            self.vyraz(el.nazev).match(self.vyraz(self.form.txt)) ||
            self.form.txt == ""
            //        ||
      ...
      if (self.Col.hasOwnProperty("DataPrace") && self.Col.DataPrace.length) {
        ...
        self.Col.DataPrace.forEach(el => {
          ...
          if (self.Col.hasOwnProperty("DataDod") && self.form.itemSelectedDod) {
            idx = _.indexOf(
              el.dod_seznam,
              self.form.itemSelectedDod.idefix_firma
            ); //console.log(el.nazev);
          ...
          if (idx >= 0 && self.form.txtPrace == "") {
          ...
          } else if (!zadanDod && self.form.txtPrace == "") {
            //        el.prace+= "/ B " + idx
            neco.push(el);
            self.filtrDataPrac.push(el);
            self.form.showtxtPrace = true;
            ...
            !zadanDod &&
            self.form.txtPrace > "" &&
            self.vyraz(el.prace).indexOf(self.vyraz(self.form.txtPrace)) >= 0
    ...
    async filtrDataDod() {
      ...
      if (self.Col.hasOwnProperty("DataDod")) {
        self.Col.DataDod.forEach(el => {
          if (
            self.Col.hasOwnProperty("DataPrace") &&
            self.form.itemSelectedPrace
          ) {
            idx = _.indexOf(
              el.prace_seznam,
              self.form.itemSelectedPrace.idefix_prace
            ); //console.log(el.nazev);
          }
          if (
            (!self.form.itemSelectedPrace || self.form.txtPrace == "") &&
            self.form.txtDod == ""
          ) {
            neco.push(el);
            ...
            (!self.form.itemSelectedPrace || self.form.txtPrace == "") &&
            self.form.txtDod > "" &&
            self.vyraz(el.firma).indexOf(self.vyraz(self.form.txtDod)) >= 0
          ) {
            neco.push(el);
            self.filtrDataDo.push(el);
            self.form.showtxtDod = true;
          } else if (idx >= 0 && self.form.txtDod == "") {
            neco.push(el);
            self.filtrDataDo.push(el);
            self.form.showtxtDod = true;
          } else if (
      ...
      if (self.Col.hasOwnProperty("dataStroj")) {
        self.Col.dataStroj.forEach(el => {
          ...
          if (self.form.txtStroj == "") {
            ...
            self.form.txtStroj > "" &&
            self.vyraz(el.nazev).indexOf(self.vyraz(self.form.txtStroj)) >= 0
            // &&  (el.nazev).toUpperCase().match(self.form.txtStroj.toUpperCase())
          ) {
            //self.form.showtxtStroj=true
            self.filtrDataStro.push(el);
      ...
      if (self.Col.hasOwnProperty("dataStroj1")) {
        ...
        self.Col.dataStroj1.forEach(el => {
          ...
          if (self.form.txtStroj1 == "") {
            ...
            self.form.txtStroj1 > "" &&
            el.nazev_stroj == self.form.txtStroj1
          ) {
            self.form.showtxtStroj1 = true;
            console.log(
              "Stejny " + self.form.txtStroj1 + " // " + el.nazev_stroj
            );
            neco = [];
            self.filtrDataStro1 = [];
            //return ;
            ...
            self.form.txtStroj1 > "" &&
            self
              .vyraz(el.nazev_stroj)
              .indexOf(self.vyraz(self.form.txtStroj1)) >= 0
            // &&  (el.nazev).toUpperCase().match(self.form.txtStroj.toUpperCase())
          ) {
            //self.form.showtxtStroj=true
            self.filtrDataStro1.push(el);
    ...
    getLeft(id, addPoz = 10, idVztah = "obal1_kalkulace") {
      ...
      if ((oNeco = document.getElementById(id))) {
        ...
        console.log(
          ...
          " Left Neco ",
          oNeco.scrollLeft,
          oNeco.parentElement.scrollLeft,
          " R2 ",
          r2.id,
          "scr ",
          r2.scrollLeft,
    ...
    getType() {
      var cRet = this.$store.getters.getIdColType(
        this.kalkulaceid,
        this.sloupecid
      );
      return cRet;

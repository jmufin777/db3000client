...
Prevod na MM bude pts * 0.352778 ?
        ...
        <tr v-for="(obr,i) in obrazky" :key="i">
          <td
            @mouseenter="odkaz=url.url()+'obrazek/'+obr.idefix;nahled=true"
            @mouseleave="nahled=true"
          ...
          <td style="width:35%;text-align:left;padding:10px">{{obr.stav}}</td>
        </tr>
      </table>
    </div>
  ...
  watch: {
    ...
    n: async function() {
      ...
      neco = (
        await Q.all(
          self.idefix,
          "select * from prilohy_prijem order by idefix desc limit 5"
        )
      ).data.data;
      this.obrazky = neco;
    }
  ...
  methods: {
    async smaz(obrazek_id) {
      ...
      if (smaz == true) {
        ...
        await self.otazka("Soubor je pridelen k zakazce", "Vymazat ?")
      ) {
        obrazek.del(self.idefix, obrazek_id);
      }
    },
    ...
    obrazek() {
      ...
      obrazek.all(self.idefix).then(obr => {
        self.obrazek1 = obr;
    ...
    async handleFileUpload(evt) {
      ...
      this.obrazky = (
        await Q.all(
          self.idefix,
          "select * from prilohy_prijem order by idefix desc"
        )
      ).data.data;
      ...
      for (var i = 0, fi; (fi = files[i]); i++) {
        self.output.push(
          ...
          " bytes, last modified: ",
            ...
            ? fi.lastModifiedDate.toLocaleDateString()
            : "n/a",
          "</li>"
        );
      }

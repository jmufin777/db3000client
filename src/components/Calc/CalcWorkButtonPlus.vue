<template>
  ...
  <div
    ...
    class="green lighten-5 pa-0 pt-0 pb-0"
    id="test_1"
    @scroll="TestRend"
  >
    <div v-if="false">I:{{IDEFIX}} {{ ID }} {{ ID2 }}</div>
    <table style="float:left">
      <tr>
        <td
          style="border-top:none;border-bottom:none;border-right: solid 2px white;max-width:5.5em;text-align:center"
        >
          <div
            class="honza_color"
            style="height:26px;padding-top:2px;text-align:center;width:5.2em;width:100%"
          >
    ...
    <div
      class="white"
      style="height:26px;padding-top:2px;text-align:left;padding-left:7px;width:100%"
    >&nbsp;</div>
  </div>
  <!-- </div> -->
</template>
...
export default {
  ...
  methods: {
    ...
    Rozdel() {
      ...
      var stBackup = stAll.filter((el, idx2) => {
        //Zalozit do db
        return idx2 < self.ID;
      });

      var stNew = stAll.filter((el, idx3) => {
        //Zbytek pro praci
        return idx3 >= self.ID;
      });

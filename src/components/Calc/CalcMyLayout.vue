<template>
  ...
  <div
    v-if="ukaz==true && ID > 0"
    class="lighten-5 pt-0 px-0 mt-0 py-0"
    :style="'height:'+getHeight('d1_'+ID,100)+'px'"
    :id="'d1_'+ID"
    @click="s12(1,$event)"
    @scroll="TestRend"
  ...
  >
    ...
    <table cols="100">
      ...
      <tr>
        <td colspan="90">
          <slot name="hlavni"></slot>
        </td>
        <!-- Hlavni nabidka -->
        <td colspan="10" style="text-align:center">
          <slot name="menuleft">Menu Leve</slot>
        </td>
      </tr>
      <tr>
      ...
      <tr class="pa-0">
        <td
          colspan="100"
          :style="'height:'+getHeight('d1_'+ID,s1)+'px'"
          class="pa-0"
          :id="'d2_'+ID"
        >
          <div
            class="green lighten-5 pt-0 py-0 px-0 mt-0 ml-2"
            style="position:relative;overflow:scroll;max-height:100%"
            id="obal1_kalkulace"
          >
      ...
      <tr v-if="false" :style="'height:'+getHeight('d1_'+ID,50)+'px'" class="pt-0">
        <td colspan="100">
          <div
            v-if="false"
            class="orange lighten-5 pt-0 px-0 mt-0"
            :id="'d3_'+ID"
            :style="'height:'+getHeight('d1_'+ID,s2)+'px'"
            style="overflow:auto"
            @click="s12(2,$event)"
          >
...
export default {
  ...
  methods: {
    ...
    s12(prvek, e) {
      ...
      document.getElementById("d1_" + self.ID).style.height =
        getHeight("d1_" + ID, s1) + "px";
      document.getElementById("d2_" + self.ID).style.height =
        getHeight("d1_" + ID, s1) + "px";
      document.getElementById("d3_" + self.ID).style.height =
        getHeight("d1_" + ID, s2) + "px";
    },

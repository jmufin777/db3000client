  <template>
<el-row id="m341" style="overflow:scroll"  class="pa-2" >
  <el-col :span="24">
  <el-row  :gutter="100">
  ...
  <el-input prefix-icon="el-icon-search" id="search_341" autofocus clearable size="mini" v-model="search" placeholder="Prohledat tabulku">
  </el-input>
  </el-col>
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t341">
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t342">
  <form id="f341">
  ...
  >
      ...
      v-show="col.props.visible=='yes'"
      >
      <div :id="'d342_r_'+irow+'_c_'+icol"  class='dcell' >

        <input type="number" v-if="col.type =='number'"
        class=" px-4 cell " :id="'c342_r_'+irow+'_c_'+icol"
       ...
       :value="item[col.id]"  style="width:100%;border:none;height:100%;text-align:left" readonly
        ...
        class="white px-4 cell seda" :id="'c342_r_'+irow+'_c_'+icol"
        v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
        :value="item[col.id]" style="width:100%;border:none;height:100%" readonly

        min="2007-06-01T08:30" max="3420-06-30T16:30"
        >
...
import List2MatDostupnost from '@/services/List2MatDostupnostService'
...
//    return
    if (this.isUserLoggedIn) {
      this.IsWaiting = true
      this.list = (await List2MatDostupnost.all(this.user,'nic')).data
...
setTimeout(function(){
    document.getElementById("m341").style.height=Math.round(window.innerHeight - 110)  + "px"
    document.getElementById("t341").style.height=Math.round(window.innerHeight - 140)  + "px"
    document.getElementById("t342").style.height=Math.round(window.innerHeight - 270)  + "px"

    document.getElementById("t342").addEventListener('keydown', (function(e) {

             self.obsluha(e, e.target)
      }))

    document.getElementById("t342").addEventListener('click', (function(e) {
  ...
  destroyed () {
    if (document.getElementById("t342")){
        ///nejde
      //document.getElementById("t342").removeEventListener(document.getElementById("t342"),'keydown')

    }
    // alert('destos'+document.getElementById("t342"))
  },
  beforeUpdate () {
...
copyLine(nRow) {
   ...
   async saveLines(id){
     ...
     await List2MatDostupnost.insert(this.user, {data: Posli, del: aDel })
     .then (res => {
   ...
   async newLine (nRow)  {
     var x
      ...
      this.Max = (await List2MatDostupnost.all(this.user,'max')).data[0].kod*1 +10
   ...
   deleteLine(nRow) {
    ...
    //  return
     ...
     if (eof == true) {
       new_id='c342_r_'+(self.list.length -1)+'_c_'+1
     }
     if (top == true) {
       new_id='c342_r_'+(0)+'_c_'+1
     }

    if (next == true) {
       new_id='c342_r_'+(nRow)+'_c_'+1
     }
...
//       this.$alert(ekeyCode)
   ...
   //return
    if (document.getElementById('search_341') && ekeyCode == 114 ){
       document.getElementById('search_341').focus()
     }
...
*/
   ...
   currid (itemId, colid) {
    ...
    async appendRow(index,rows) {
      ...
      try {
        await (List2MatDostupnost.insert(this.user, this.form))

      } catch (err) {
        console.log(err)
      }

      await List2MatDostupnost.all(this.user,'nic')
      .then(res => {

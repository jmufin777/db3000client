  <template>
<el-row id="m391" style="overflow:scroll"  class="pa-2" >
  <el-col :span="24">
  <el-row  :gutter="100">
  ...
  <el-input prefix-icon="el-icon-search" id="search_391" autofocus clearable size="mini" v-model="search" placeholder="Prohledat tabulku">
  </el-input>
  </el-col>
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t391">
...
<div style="height:100%;overflow:scroll" class="mt-0" id="t392">
  <form id="f391">
  ...
  >
      ...
      v-show="col.props.visible=='yes'"
      >
      <div :id="'d392_r_'+irow+'_c_'+icol"  class='dcell' >

        <input type="number" v-if="col.type =='number'"
        class=" px-4 cell " :id="'c392_r_'+irow+'_c_'+icol"
       ...
       >
        ...
        class="white px-4 cell seda" :id="'c392_r_'+irow+'_c_'+icol"
        v-bind:class="{seda: irow % 2 ==0 , bila:  irow % 2 >0}"
        :value="item[col.id]" style="width:100%;border:none;height:100%" readonly

        min="2007-06-01T08:30" max="3920-06-30T16:30"
        >
...
import List2StrojTiskMod from '@/services/List2StrojTiskModService'
// import List2StrojTiskModVue from './List2StrojTiskMod.vue';
...
//    return
    if (this.isUserLoggedIn) {
      this.IsWaiting = true
      this.list = (await List2StrojTiskMod.all(this.user,'nic')).data
...
setTimeout(function(){
  ...
  destroyed () {
    if (document.getElementById("t392")){
        ///nejde
      //document.getElementById("t392").removeEventListener(document.getElementById("t302"),'keydown')
...
copyLine(nRow) {
   ...
   async saveLines(id){
     ...
     await List2StrojTiskMod.insert(this.user, {data: Posli, del: aDel })
     .then (res => {
   ...
   async newLine (nRow)  {
     var x
      ...
      this.Max = (await List2StrojTiskMod.all(this.user,'max')).data[0].kod*1 +10
...
//       this.$alert(ekeyCode)
   ...
   //return
    if (document.getElementById('search_391') && ekeyCode == 114 ){
       document.getElementById('search_391').focus()
     }
...
*/
   ...
   currid (itemId, colid) {
    ...
    async appendRow(index,rows) {
      ...
      try {
        await (List2StrojTiskMod.insert(this.user, this.form))

      } catch (err) {
        console.log(err)
      }

      await List2StrojTiskMod.all(this.user,'nic')
      .then(res => {
